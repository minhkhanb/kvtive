...
+function ($) {
    ...
    var Validator = function (element, options) {
        ...
        this.$element.find('[data-compare]').each(function () {
            var $this = $(this)
            var target = $this.attr('data-compare')

            $(target).on('input.bs.validator', function (e) {
                compareDate($this.val(), target.val()) && $this.trigger('input.bs.validator')
            })
        });

        this.$element.find('[data-range]').each(function () {
            var $this = $(this);
            var target_from = $this.attr('data-range-from');
            var target_to = $this.attr('data-range-to');

            $(target_from).on('input.bs.validator', function (e) {
                compareRangeDate($this.val(), $(target_from).val(), $(target_to).val()) && $this.trigger('input.bs.validator')
            });
            $(target_to).on('input.bs.validator', function (e) {
                compareRangeDate($this.val(), $(target_from).val(), $(target_to).val()) && $this.trigger('input.bs.validator')
            });
        });
